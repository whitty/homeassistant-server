...
def test_parse_library_album(provider: YoutubeMusicProvider) -> None:
    ...
    assert album.artists[0].media_type == MediaType.ARTIST
    assert len(album.metadata.images) == 1
    assert album.metadata.images[0].type == ImageType.THUMB
    assert 'Elasticity is the third EP' in album.metadata.description
...
def test_parse_uploaded_album(provider: YoutubeMusicProvider) -> None:
    ...
    assert album.artists[0].name == 'Triple J'
    assert album.artists[0].media_type == MediaType.ARTIST
